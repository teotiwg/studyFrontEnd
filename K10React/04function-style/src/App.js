...
function App() {
  ...
  해당 함수가 변수의값을 변경할 수 있으므로 아래 2개의 훅은 
...
function FuncComponent(props){
  ...
  //return전에 특정 함수를 호출하는것으로 전처리를 할 수 있다. 
...
class ClassComponent extends React.Component {
  ...
  /*
  최초로 렌더링될때는 호출되지 않고, state값 변경에 의한 재 렌더링이
  될때 호출된다. 해당 함수에서 true가 호출될때만 render()를 호출할수
  있다. 만약 false가 반환되면 render()가 호출되지 않아 화면이 갱신되지
  않는다. 
  */
